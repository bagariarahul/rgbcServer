...
class CloudBackupServer {
    ...
    setupBasicMiddleware() {
        ...
        // Native clients (Android app, Python master) send NO Origin header —
        // allow those. Reflecting arbitrary browser origins with credentials is
        // a hole; only allow our own domain for any browser-based access.
        ...
        this.app.use(cors({
            origin: (origin, callback) => {
                // No origin = native app / server-to-server / curl → allow.
        ...
        // Strict limiter for auth endpoints — brute-force protection. The global
        // 1000/15min is too loose to stop credential/token guessing.
        ...
        // Apply where auth routes are mounted:
    ...
    setupAdvancedRoutes() {
        ...
        try {
            ...
            logger.warn(
                '❌ verifyToken middleware not found, routes will be unprotected'
            );
        ...
        const loadRoute = (pathStr, requirePath) => {
            try {
                ...
                if (typeof routeModule === 'function') {
                    ...
                    logger.warn(
                        `❌ Module ${requirePath} is not a valid Router middleware`
                    );
                }
            } catch (e) {
